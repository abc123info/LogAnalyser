...
2026.06.28 新增 安全排行：攻击源 IP / 攻击地域 / 404 热点 URL Top10 条形图；404 图整行展示并优化 URL 路径显示。

2026.06.26 新增 运营流量：访问/流量地域 Top10、搜索引擎与爬虫占比饼图。

2026.06.25 新增 安全趋势：攻击类型×小时堆叠面积图、攻击 vs 请求双轴对比、静态资源 POST 异常 Top10、可疑/爬虫 UA 细分；修复小时键不一致导致时序图为空的问题。

2026.06.24 修复了 重复编译正则、每行分配byte[]、UA解析两次造成的性能消耗问题。

2026.06.23 修复了 GeoIP 查询全局锁和地域信息重复查询/格式化的造成的性能瓶颈问题。

2026.06.22 修复了 启动静态加载造成的UI上"只分析动态 URL"等选项在二次分析时不能正确生效的bug。
